...
def test_normalize_document_file_name():
    ...
    with pytest.raises(ValueError, match="cannot be null"):
        normalize_document_file_name(None)

    with pytest.raises(ValueError, match="cannot be empty"):
        normalize_document_file_name("   ")
...
class TestIngestJsonData:
    ...
    def test_ingest_json_data_rejects_null_metadata_file_name(self, mock_neo4j_graph, sample_json_data):
        sample_json_data["metadata"]["file_name"] = None
        ...
        assert not any("file_name" in p for p in merged_params)
